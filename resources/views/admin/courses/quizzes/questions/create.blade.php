...
@section('content')
    ...
    removeOption(index) { if (this.options.length > 2) this.options.splice(index, 1) }
        ...
        <h1 class="text-3xl font-bold mb-6">Tạo câu hỏi mới cho {{ $quiz->title }}</h1>
        <form action="{{ route('admin.courses.sections.quizzes.questions.store', [$course, $section, $quiz]) }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Câu hỏi</label>
                <textarea name="question" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('question') }}</textarea>
                @error('question')
            ...
            <div class="mb-4">
                ...
                <select name="type" x-model="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    <option value="single_choice" {{ old('type') == 'single_choice' ? 'selected' : '' }}>Trắc nghiệm một đáp án</option>
                    <option value="multiple_choice" {{ old('type') == 'multiple_choice' ? 'selected' : '' }}>Trắc nghiệm nhiều đáp án</option>
                    <option value="fill_blank" {{ old('type') == 'fill_blank' ? 'selected' : '' }}>Điền vào chỗ trống</option>
            ...
            <div class="mb-4">
                ...
                <textarea name="explanation" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('explanation') }}</textarea>
                @error('explanation')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" name="points" value="{{ old('points', 1) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                @error('points')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="checkbox" name="is_required" value="1" {{ old('is_required') ? 'checked' : '' }} class="mt-1">
                @error('is_required')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
